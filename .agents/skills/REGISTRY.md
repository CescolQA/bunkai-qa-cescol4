...
> Generated: `2026-09-17T17:31:29.191Z`
...
**Purpose**: Find every LLM workflow in the current repository and label it, so Caveman Cloud groups spend by what the code actually does (support-rep...
...
> Source: `.agents\skills\caveman-discover\SKILL.md` · phase: `unknown` · extraction strategy: B
...
**Purpose**: Review Caveman Cloud evidence read-only: costs, Cave Score, Cave Plan, workflows, traces, latency, errors, compression, routing, and veri...
...
- evidence cost.
- Do not fetch prompt, completion, tool, or artifact payloads unless the user
- Scope every read to the project selected by Caveman context. Never supply an
- Empty results are evidence of no current signal, not zero cost or zero risk.
- Cite trace ids and exact time windows used. Do not claim a cause from an
- `overview`
- `costs`
- `score`
- `workflows`
...
> Source: `.agents\skills\caveman-evidence-review\SKILL.md` · phase: `unknown` · extraction strategy: B
...
**Purpose**: Read-only repository explorer.

**Compact Rules**:
- Issue several tool calls IN PARALLEL in your first turn — cast a broad net.
- Follow the evidence over one or two more turns only if needed. Stop as soon as
- Only cite line ranges you actually read. Never invent or estimate a range, and

**Read full SKILL.md when**: the compact rules above are insufficient (e.g. novel scenario, debugging, or the briefing tells you to load the full skill).

> Source: `.agents\skills\caveman-explore\SKILL.md` · phase: `unknown` · extraction strategy: B
...
**Purpose**: Close the loop on a Caveman learn report — review the ranked token sinks and apply cost-lowering fixes (trim config, offload recurring co...

**Compact Rules**:
- Run: caveman learn report --json
...
> Source: `.agents\skills\caveman-learn\SKILL.md` · phase: `unknown` · extraction strategy: B
...
**Purpose**: Inspect Caveman Cloud's eval-gated experiment lifecycle and block unsafe execution.

**Compact Rules**:
- A request to review, inspect, explain, or recommend authorizes reads only.
- Never approve an experiment whose results are pending, whose required
- Never convert experiment lift into `verified_savings`. Only active real
- Never supply an organization id. Project and tenant scope come from the
- Never execute a lifecycle mutation, even after user approval. Exact
- Unknown states and server errors fail closed. Report exact
...
> Source: `.agents\skills\caveman-manage\SKILL.md` · phase: `unknown` · extraction strategy: B
...
**Purpose**: Turn Caveman's exact report-only repository observations into an operator-chosen optimization candidate with a paired baseline/candidate...
...
> Source: `.agents\skills\caveman-optimize\SKILL.md` · phase: `unknown` · extraction strategy: B
...
**Purpose**: Wire the current repository through the Caveman Cloud gateway so every LLM request is measured — cost, tokens, latency — with zero behavi...
...
- **Coherent integration.** Wire every live LLM callsite through existing
- **Secrets stay in env vars.** `CAVE_API_KEY` goes into the env file the repo
- **Report only what you observed.** The final report states the HTTP status
- **Record mode only.** You are adding measurement. You do not enable any
- **Provider keys are not your business.** With `PROVIDER_KEYS: stored` you
...
> Source: `.agents\skills\caveman-setup\SKILL.md` · phase: `unknown` · extraction strategy: B
...
**Purpose**: Diagnose ambiguous failures before editing.
...
> Source: `.agents\skills\investigate-first\SKILL.md` · phase: `unknown` · extraction strategy: B
...
**Purpose**: Build feature work with high overbuilding risk.
...
> Source: `.agents\skills\lean-build\SKILL.md` · phase: `unknown` · extraction strategy: B
...
**Purpose**: Implement reversible compatibility-safe transitions.
...
> Source: `.agents\skills\migration\SKILL.md` · phase: `unknown` · extraction strategy: B
...
**Purpose**: Live snapshot of this project's configuration — compiled fresh every run from .agents/project.yaml, .env, .mcp.json, and the Jira QA cred...
...
> Source: `.agents\skills\project-status\SKILL.md` · phase: `unknown` · extraction strategy: A
...
**Purpose**: Restructure code while preserving behavior.
...
> Source: `.agents\skills\safe-refactor\SKILL.md` · phase: `unknown` · extraction strategy: B
...
**Purpose**: Fix bugs and small behavior changes at the narrowest responsible layer.
...
> Source: `.agents\skills\surgical-patch\SKILL.md` · phase: `unknown` · extraction strategy: B
...
**Purpose**: Prove existing work meets acceptance conditions without expanding scope.
...
> Source: `.agents\skills\verify-and-stop\SKILL.md` · phase: `unknown` · extraction strategy: B
