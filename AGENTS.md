...
| One Claude + one OpenCode wrapper per alias (`.claude/commands/*.md`, `.opencode/commands/*.md`) | `.agents/compatibility/command-aliases.json` + optional overlay `command-aliases.project.json` | `bun run agents:compat` |

`bun run agents:compat:check` validates the whole contract: shim bytes, alias target, both wrapper sets byte-for-byte against the merged manifest, hook adapters, and MCP parity.

**COMMAND ALIASES ARE TRANSPORT, NOT WORKFLOW.** Each manifest entry names a target skill + mode; the wrapper only selects and forwards `$ARGUMENTS`. **Project overlay**: `.agents/compatibility/command-aliases.project.json` (same schema, optional, never touched by `bun run up`). Merge: upstream aliases first, overlay overrides by `alias` name or adds new ones. This project uses the overlay for `/project-status` (alias → skill `project-status` mode `status`).
...
| `project-status` | `/project-status` | Live snapshot of this project's configuration, compiled fresh every run from `.agents/project.yaml` + `.env` (names only, secrets masked) + `.mcp.json` + Jira QA credentials epic + `git status`. Never cached, never written to disk. Optional `$ARGUMENTS` scopes to one section (`mcp`, `env`, `jira`, `db`, `git`). Project-owned: wired via the command-alias overlay (§4.5), not upstream. |
...
| `/project-status` | `project-status` mode `status` (project overlay) |
...
**HIGHER-ALTITUDE SWEEP**: FTP / STP / STR sit ABOVE a Story, so the coverage walk structurally cannot reach them: an unfiltered `pull` ALSO sweeps the CHILDREN of the four QA-process Epics (resolved by the `QA-Artifact` label → cached `qa.qa_epics.*.key` → `QA ` name prefix; no new config), materializing the higher-altitude Plans and Runs plus Test Sets and Preconditions into the dirs above. Coverables, Tests and Story-altitude `ATP:` Plans are excluded: each already has a canonical home. Skip with `--no-qa-artifacts`; a project with no QA-process Epics runs zero extra queries.
...
>
> `git_strategy.strategy` ships **`solo-main`**, not null. That is a DEFAULT, not a decision on its own, and `meta.strategy_source` (`inherited` vs `chosen`) is what records whether it was actually confirmed for THIS project. `git-flow-master` OFFERS "Strategy Setup" when a project has filled in its `project_name` and `strategy_source` is still `inherited`: a real project running a strategy nobody chose. `.agents/project.yaml` is frozen by `bun run up` (updater `bootstrapOnlyPaths`), so every project keeps its own. Downstream test-automation projects typically choose `sdet` (chained suites; see `.agents/skills/git-flow-master/references/sdet-integration-trunk.md`).

This repository (César's personal QA-practice project, built on the agentic-qa-boilerplate) runs `solo-main`: single maintainer, commit and push directly to `main`. This is a CHOSEN strategy (`strategy_source: chosen`, confirmed 2026-09-17 via `/git-flow-master` Strategy Setup — single-branch mechanics already matched real usage), not the inherited default: do not re-offer Strategy Setup unless the user asks to change it. No GitHub ruleset governs `main` on this repo (`bun run git:policy verify` confirms "no ruleset rules apply", 0 drift, `meta.policy_source: verified`): there is no accepted divergence to declare.
